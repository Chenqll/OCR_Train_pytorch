...
import numpy as np
from detect.ctpn_predict import CTPN_PREDICT
from recognize.crnn_recognizer import PytorchOcr
import os

class OCR:
    def __init__(self):
        self.recognizer = PytorchOcr()

    def ocr(self, image_file,result_dir):
        # 预处理
        image = np.array(Image.open(image_file).convert('RGB'))
        # detect
        text_recs, img_framed, image = CTPN_PREDICT().get_det_boxes(image)
...
if __name__ == "__main__":
    ...
    parser.add_argument('result_dir', help="Path to the image. RGB one will be converted to grayscale")
    args = parser.parse_args()
    model = OCR()
    result, image_framed = model.ocr(args.image,args.result_dir)
    print(result)